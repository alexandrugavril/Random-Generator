...
main(int argc, char *argv[])
{
	...
	if ( argc != 4 ) {
		printf("Usage: %s <stream length> <file> <summary file>\n", argv[0]);
	...
	option = generatorOptions(&streamFile, argv[2]);
	chooseTests();
	fixParameters();
...
{ 
	...
	for ( i=0; i<MAXFILESPERMITTEDFORPARTITION; i++ )
		s[i] = (char*)calloc(400, sizeof(char));
